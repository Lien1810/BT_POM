package BT7_SD_Explicitwait.page;
...
public class LoginPage extends WebUI {
    ...
    //Tao ham xay dung
    //Ham xd de truyen driver tu ben ngoai vao (Co the tu BaseTest)
    public LoginPage(WebDriver driver) {
        super(driver); // ten hàm trung vs ten class
        this.driver = driver;
    ...
    private void setEmailInput(String email) {
        wait.until(ExpectedConditions.visibilityOfElementLocated(inputEmail));//sử dụng Explicit Wait
        driver.findElement(inputEmail).sendKeys(email);
        System.out.println("Set email value: " + email);
    }

    private void setPasswordInput(String password) {
        wait.until(ExpectedConditions.visibilityOfElementLocated(inputPassword));//sử dụng Explicit Wait
        driver.findElement(inputPassword).sendKeys(password);
        System.out.println("Set password value: " + password);
    }

    private void clickbuttonLogin() {
        wait.until(ExpectedConditions.visibilityOfElementLocated(buttonLogin)); //sử dụng Explicit Wait
    ...
    public DashboardPage LoginCMS(String email, String password) {
        ...
        clickbuttonLogin();
        waitForPageLoaded(); //sử dụng Explicit Wait
